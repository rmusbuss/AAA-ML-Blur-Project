...
from blur.frontend.config import TRITON_URL, TRITON_PORT
...
class MLApi:
    ...
    def blur_image(self, image: Image, coords: list, sigma: int = 52, is_pascal_voc: bool = False):
        """
        ...
        for channel_id in range(3):
            blurred_image = Image.fromarray(image_np[:, :, channel_id]).filter(ImageFilter.GaussianBlur(sigma))
